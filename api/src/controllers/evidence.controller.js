...
class EvidenceController {
  ...
  async return(req, res) {
    try {
      ...
      // Idempotency guard: a previous attempt may have already recorded the
      // return on the server (e.g. the client timed out mid-upload). Return
      // evidence is immutable, so we must never try to re-create it. Instead,
      // reuse the existing evidence + charges, finish closing the agreement if
      // it's still open, and return success — rather than failing on the
      // immutable-evidence trigger.
      const existingReturn = await EvidenceService.getReturnEvidence(agreementId, req.tenantId);
      if (existingReturn) {
        ...
        const charges = await AutoGeneratedChargeModel.findByAgreementId(agreementId, req.tenantId);
        const agreement = await RentalAgreementModel.findById(agreementId, req.tenantId);
        if (agreement && agreement.status === 'ACTIVE') {
          await AgreementService.closeAgreement(
            agreementId, req.tenantId, req.user.id, req.ip, req.get('user-agent'), charges
          );
        }
        const totalCharges = charges.reduce((sum, c) => sum + parseFloat(c.amount), 0);
        return res.status(200).json({
          success: true,
          data: { return_evidence: existingReturn, charges, total_charges: totalCharges, already_recorded: true },
        });
      }
